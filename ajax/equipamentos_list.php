...
<div class="row">
	<div class="col-lg-8">
		<form class="form-inline" role="form" onsubmit="javascript:return(false);">
			<div class="form-group">
				...
				<input type="text" class="form-control" id="pesquisa" name="pesquisa" placeholder="Buscar por Placa/IMEI">
			</div>
	...
	<div class="col-lg-4">
		<ul class="pager">
			<li><a href="javascript:paginacao(<?=$pagina>1?$pagina-1:$pagina?>)">&larr; ANTERIOR</a></li>
			<li><a href="javascript:paginacao(<?=$pagina<$count?$pagina+1:$pagina?>)">PRÓXIMO &rarr;</a></li>
		</ul>
	</div>
</div>
	...
	<?
		...
		while($data = mysql_fetch_assoc($resEquip)):?>
			<tr>
			    ...
			    <td style="text-transform: uppercase;" id="name_<?=$data['imei']?>"><?php echo "<a href='javascript:void(0);' onclick=\"javascript:showConfirm('show'," . $data['imei'] . ");\">" . $data['name'] . "</a>"; ?></td>
			    <td style="text-transform: uppercase;" id="apelido_<?=$data['imei']?>"><?=$data['apelido'] . " / <b>" . $data['identificacao'] . "</b>" ?></td>
			    ...
			    <td style="text-transform: uppercase;" id="modelo_<?=$data['imei']?>"><?=$data['modelo_rastreador']?></td>
			    <td style="text-transform: uppercase;">
				<?php
			    	// if ($atraso || $data['status_sinal'] != 'R') echo "<img src='imagens/ignicao.png' alt='Desligado' title='Veículo desligado'> ";
					if ($data['ligado'] == 'S') echo "<img src='imagens/ligado-2.png' alt='LIGADO' title='VEÍCULO LIGADO'> ";
					else echo "<img src='imagens/desligado-2.png' alt='DESLIGADO' title='VEÍCULO DESLIGADO'> ";
			    	if ($data['bloqueado'] == 'N') echo " <img src='imagens/desbloqueado-2.png' alt='DESBLOQUEADO' title='VEÍCULO DESBLOQUEADO'>"; 
					else echo " <img src='imagens/bloqueado-2.png' alt='BLOQUEADO' title='VEÍCULO BLOQUEADO'>";			    	
			    ?>
				</td>
				<td>
				<?php
			    ...
			    <td><?=$data['ladata']?></td>
			    <td><?=$data['lahora']?></td>
			    <td><? echo "<img src='../imagens/". imagenStatusSinal($data['status_sinal'], $atraso). "'>" ?></td>
			    ...
			    <?php
			    	...
			    	echo "<a href='javascript:void(0);' title='Histórico' onclick=\"javascript:modalHistorico(".$data['imei'].",'".$data['name']."');\" ><img src='imagens/historico.png' alt='Histórico'></a> ";
					#echo "<a href='javascript:void(0);' title='$titleVel' $clickVel $estilo' onclick=\combustivel(". $data['imei'] .",',H,060');\"><img src='imagens/limit_vel.png' alt='Velocidade Limite'></a> ";
