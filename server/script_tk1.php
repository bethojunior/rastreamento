									...
									if ( $result <> $resultCerca And round($speed * 1.852,0) > 0 ) {
										...
										if ( $tipoEnvio == 0 ) {
											...
											while($data = mysql_fetch_assoc($consulta1)) {
												...
												$msg .= $mensagem ."<br /><br />";
												$msg .= "Equipe WEBARCH<br />";
												$msg .= "(91)98247-4050<br />";											    
												$msg .= "<a href=\"http://www.WEBARCH-pa.com.br\">ww.WEBARCH-pa.com.br</a></p>";
												$msg .= "</body>";
												$msg .= "</html>";
												...
												$mail->From = "modal_dados";
												$mail->FromName = "WEBARCH - PLATAFORMA DE RASTREAMENTO";
												$mail->AddAddress($emailDestino);
												$mail->AddReplyTo($mail->From, $mail->FromName);
												$mail->Subject = "WEBARCH - Alerta de Violacao de Perimetro";
												$mail->Body = $msg;
									...
									if ( $result <> $resultCerca And round($speed * 1.852,0) > 0 ) {
										...
										if ($tipoEnvio == 0) {
											...
											while($data = mysql_fetch_assoc($consulta1)) {
												...
												$msg .= "Equipe WEBARCH<br />";
												$msg .= "(85)88462069<br />";
												$msg .= "<a href=\"http://www.WEBARCH-pa.com.br\">www.WEBARCH-pa.com.br</a></p>";
												$msg .= "</body>";
												$msg .= "</html>";
...
function enviaEmail($destinatario, $assunto, $msgConteudo){
	...
	$mail->Username 	= USER_SMTP;
	$mail->Password 	= SENHA_SMTP;
	$mail->From 		= "smsoousa@gmail";
	$mail->FromName 	= "WEBARCH - PLATAFORMA DE RASTREAMENTO";
	$mail->AddAddress($destinatario);
	$mail->AddReplyTo($mail->From, $mail->FromName);
	$mail->Subject 		= "WEBARCH - Alerta ".$assunto;
